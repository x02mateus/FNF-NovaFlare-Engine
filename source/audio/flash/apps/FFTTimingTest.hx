...
* 
* @author Gerry Beauregard (original AS3 code, see note in audio.fft.FFT)
* @author Rigondo (Haxe version, http://rigondo.wordpress.com/2011/08/18/9/)
* @author Konstantin Tretyakov (some modifications)
*/
class FFTTimingTest {
    static inline var LOG_N = 10;           // Log2 of the FFT length
    static inline var N: UInt = 1 << LOG_N;     // FFT Length (2^LOG_N)
    ...
    public function new() {
        ...
        var xRe: HxaArray<Float> = HxaArrayUtil.newArray(N);
        var xIm: HxaArray<Float> = HxaArrayUtil.newArray(N);
    ...
    inline function checkIfHaxeTranslationIsCorrect() {
        ...
        // copies for test:
        var xRe2: HxaArray<Float> = HxaArrayUtil.newArray(N);
        var xIm2: HxaArray<Float> = HxaArrayUtil.newArray(N);
