...
class ResultRect extends FlxSpriteGroup
{
    ...
    public function updateRect(msGroup:Array<Float> = [], timeGroup:Array<Float> = [])
    {
        ...
        shape.graphics.drawRect(0, _height / 2 - (ClientPrefs.data.marvelousWindow / safeZoneOffset) * _height / 2 - 1, _width, 2);
        shape.graphics.drawRect(0, _height / 2 + (ClientPrefs.data.marvelousWindow / safeZoneOffset) * _height / 2 - 1, _width, 2);
        shape.graphics.endFill();
        
        shape.graphics.beginFill(colorArrayAlpha[1]); 
        shape.graphics.drawRect(0, _height / 2 - (ClientPrefs.data.sickWindow / safeZoneOffset) * _height / 2 - 1, _width, 2);
        shape.graphics.drawRect(0, _height / 2 + (ClientPrefs.data.sickWindow / safeZoneOffset) * _height / 2 - 1, _width, 2);
        shape.graphics.endFill();
        
        shape.graphics.beginFill(colorArrayAlpha[2]); 
        shape.graphics.drawRect(0, _height / 2 - (ClientPrefs.data.goodWindow / safeZoneOffset) * _height / 2 - 1, _width, 2);
        shape.graphics.drawRect(0, _height / 2 + (ClientPrefs.data.goodWindow / safeZoneOffset) * _height / 2 - 1, _width, 2);
        shape.graphics.endFill();
